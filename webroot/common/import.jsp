...
function showErrorMsgButton(){
    ...
    $('#reloadExcel').bind('click',function(){
        ...
        $("#<s:property value='#parameters.dialog'/>").createDialog({
            ...
            height: 220,
            title:'数据导入',
            url:"${path}/common/import.jsp?dataType=<s:property value='#parameters.dataType'/>&dialog=<s:property value='#parameters.dialog'/>&startRow=<s:property value='#parameters.startRow'/>&templates=<s:property value='#parameters.templates'/>",
            buttons:{
                "重新导入" : function(event){
                      clearInterval(intarcalId);
                      $("#mForm").submit();
