...
public class ThreeRecordsIssuePeriodCompleteState extends
	...
	@Override
	public ThreeRecordsIssueStepGroup back(BaseWorking baseWork,
		...
		if (currentStep == null || currentStep.getTarget() == null
				|| context == null || baseWork == null
				|| baseWork.getCreateOrg() == null) {
			throw new ThreeRecordsIssueOperationException("参数未获得！");
		}
		if (!getLoginedOrg(context).equals(baseWork.getCreateOrg())) {
		...
		return backIssueToSourceStep(baseWork, currentStep, context);
	}
	...
	private ThreeRecordsIssueStepGroup backIssueToSourceStep(BaseWorking issue,
		...
		if ("true".equals(GridProperties.IS_NEED_UNCONCEPTEDSTATE)) {
			...
			// 如果是在本级则设置为待 办
			keyStep.setState(ThreeRecordsIssuePeriodCompleteState.class
					.getName());
	...
	private ThreeRecordsIssueStepGroup transferIssueToOtherOrg(
			BaseWorking baseWork, ThreeRecordsIssueStep currentStep,
			Organization loginedOrg, Organization target,
			List<Organization> tellOrgs, boolean tellHighOrg,
			List<Organization> noticeOrgs) {
		if (baseWork == null || baseWork.getId() == null || currentStep == null) {
			throw new ThreeRecordsIssueOperationException("参数未获得！");
		}
		ThreeRecordsIssueStepGroup result = new ThreeRecordsIssueStepGroup();
		ThreeRecordsIssueStep keyStep = constructIssueStepAndFillDefaultProperty(
				baseWork, loginedOrg, target);
		keyStep.setBackTo(currentStep);
		result.setLedgerId(baseWork.getId());
		result.setLedgerType(baseWork.getLedgerType());
		...
		result.setKeyStep(keyStep);
		baseWork.setCurrentStep(keyStep);
		baseWork.setCurrentOrg(keyStep.getTarget());
		baseWork.setStatus(DEALING);
		return result;
	}
	...
	public ThreeRecordsIssueStepGroup periodComplete(BaseWorking baseWork,
		...
		return transferIssueForPeriodComplete(baseWork, currentStep,
	...
	private ThreeRecordsIssueStepGroup transferIssueForPeriodComplete(
			...
			Organization loginedOrg, Organization target) {
		if (issue == null || currentStep == null
				|| currentStep.getLedgerId() == null) {
			throw new ThreeRecordsIssueOperationException("参数未获得！");
		}
		ThreeRecordsIssueStepGroup result = new ThreeRecordsIssueStepGroup();
		ThreeRecordsIssueStep keyStep = constructIssueStepAndFillDefaultProperty(
