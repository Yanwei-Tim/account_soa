...
function extendDefaultOption(defaultOption, option, colModelFromCookie){
	defaultOption.height = $(".ui-layout-west").outerHeight()-$("#nav:visible").outerHeight()-$("#thisCrumbs:visible").outerHeight()-65;
...
;(function ($) {
	...
	$.fn.jqSubGrid=function(option){
		...
		defaultOption.height = $(".ui-layout-west").outerHeight()-$(".tabnav").outerHeight()-$(".btnbanner").outerHeight()-$(".tabbox").outerHeight()-$("#nav").outerHeight()-85;
		if(defaultOption.pager==false){
			defaultOption.rowNum=-1;
		}
		$.extend(defaultOption,option);
	...
	$.fn.jqTreeGrid=function(option){
		...
		defaultOption.height = $(".ui-layout-west").outerHeight()-$(".path:visible").outerHeight()-$("#nav:visible").outerHeight()-$(".submenu:visible").outerHeight()-75;
		extendDefaultOption(defaultOption,option,getColModelFromCookie(self));
		if(option.loadComplete){
			defaultOption.loadComplete=function(){
				option.loadComplete();
	...
	$.fn.jqMailUtil=function(option){
		...
		var defaultOption={
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$(".subgrid-cell").remove();
				$("#"+subgrid_id).parent().attr("colspan",colspan+1);
				$.extend(defaultOption.subGridPostData,{id:row_id});
				self.find(".sgexpanded").click();
				$("#"+subgrid_id).load(defaultOption.subGridUrl,defaultOption.subGridPostData,function(){
					var scrHeight=0;
					self.find("tr").each(function(i,n){
						if($(n).attr("id")==row_id){
							return false;
			...
			subGridRowColapsed: function(subgrid_id, row_id) {
				...
				row.find("td:last").css({"border-right-width":"1px","border-left-width":"1px","border-right-color":selectColor()});
			},
			ondblClickRow : function(rowid){
				self.toggleSubGridRow(rowid);
			},
			caption:false,
			gridComplete:function(){
				//$("tr",self).find("td:not(:first):not(:last)").css({"border-left":"1px","border-right":"1px"})
			},
			loadComplete:function(){
				$.loadingComp("close");
			}
		...
		defaultOption.height = $(".ui-layout-west").outerHeight()-$(".tabnav").outerHeight()-$(".tabbox").outerHeight()-$("#nav").outerHeight()-85;
		if(defaultOption.pager==false){
			defaultOption.rowNum=-1;
		}
		$.extend(defaultOption,option);
